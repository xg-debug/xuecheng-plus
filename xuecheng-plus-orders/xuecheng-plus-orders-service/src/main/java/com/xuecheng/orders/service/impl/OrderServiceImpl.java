...
public class OrderServiceImpl implements OrderService {
    ...
    public PayRecordDto queryPayResult(String payNo) {
        ...
        if(payRecord == null) {
            XueChengPlusException.cast("请重新点击获取支付二维码");
        }
    ...
    public void saveAliPayStatus(PayStatusDto payStatusDto) {
        ...
        if(trade_status.equals("TRADE_SUCCESS")) {
            ...
            payRecord_u.setStatus("支付成功");
            payRecord_u.setOutPayChannel("Alipay");
            payRecord_u.setOutPayNo(payStatusDto.getOut_trade_no()); // 支付宝交易号
            payRecord_u.setPaySuccessTime(LocalDateTime.now()); // 通知时间
            int update = payRecordMapper.update(payRecord_u, new LambdaQueryWrapper<XcPayRecord>().eq(XcPayRecord::getPayNo, payNo));
            if(update > 0) {
            ...
            int update_o = ordersMapper.update(order_u, new LambdaQueryWrapper<XcOrders>().eq(XcOrders::getId, orderId));
            if(update_o > 0) {
    ...
    public XcOrders saveXcOrders(String userId, AddOrderDto addOrderDto) {
        ...
        String orderDetailJson = addOrderDto.getOrderDetail(); // 可能有多件商品
    ...
    public XcPayRecord createPayRecord(XcOrders orders) {
        if(orders == null) {
            XueChengPlusException.cast("此订单不存在");
        }
        if(orders.getStatus().equals("600002")) {
            XueChengPlusException.cast("订单已支付");
        }
